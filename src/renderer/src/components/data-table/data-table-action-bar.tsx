...
function DataTableActionBar<TData>({
  ...
  const portalContainer =
    portalContainerProp ??
    (mounted
      ? (globalThis.document?.getElementById('main-content') ?? globalThis.document?.body)
      : null)
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className={cn(
            'absolute inset-x-105 bottom-10 z-50 flex w-fit flex-wrap items-center justify-center gap-2 rounded-md border bg-background p-2 text-foreground shadow-sm',
            className
          )}
          {...props}
        >
